...
//
//   node --env-file=.env scripts/testRedHillsCall.js <phone-number-E164>
//   Example: node --env-file=.env scripts/testRedHillsCall.js +919876543210
//
// Running plain `node scripts/testRedHillsCall.js ...` without --env-file
...
const localeOverride = process.argv[3];
if (!driverPhone) {
  console.error(
    "Usage: node --env-file=.env scripts/testRedHillsCall.js <phone-number-E164> [locale-override]\n" +
      "  locale-override is diagnostic-only (e.g. en-IN) — doesn't change the config's real hi-IN setting.",
  );
  process.exit(1);
}
...
await runCallAndLog(client, {
  ...
  recipient: {
    ...
    locale: localeOverride || config.language.locale,
  },
  resultSchema,
});